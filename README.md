...
```go
import geoapi "github.com/geosearch-dev/geoapi-go"
```
...
For using other server than the one defined on index 0 set context value `geoapi.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), geoapi.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `geoapi.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), geoapi.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `geoapi.ContextOperationServerIndices` and `geoapi.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), geoapi.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), geoapi.ContextOperationServerVariables, map[string]map[string]string{
...
auth := context.WithValue(
		context.Background(),
		geoapi.ContextAPIKeys,
		map[string]geoapi.APIKey{
